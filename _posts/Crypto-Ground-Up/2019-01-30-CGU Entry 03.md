...
Now we're going to look at a very basic example. So, here goes. Let's say that `a=13`, `m=9` and we want to find out what `r` is which we'll currently mark with a `?`.
...
What we need to do is work out how many times 9 goes into 13, which as I'm sure you know is one time - which leaves us with a remainder of 4 which we can write as: $13 ≡ 4 mod 9$
